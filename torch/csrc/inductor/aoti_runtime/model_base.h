...
class AOTInductorModelBase {
 public:
  ...
  std::unordered_map<std::string, AtenTensorHandle> run_const_fold(
    ...
    // fallible calls below (cudaEventRecord / XPU barrier / wait_for_completion)
    // can throw; without cleanup the map's destructor drops those raw handles
    // without freeing the underlying tensors, leaking folded-constant GPU memory
    // (the container catches and keeps serving, so it accumulates). Free the
    // still-owned handles on the error path only. This header is compiled into
    // model.so, so use only the stable C ABI (no c10 scope-guard).
    try {
#ifdef USE_CUDA
      AOTI_RUNTIME_CUDA_CHECK(cudaEventRecord(*run_finished_, stream));
#elif defined(USE_XPU)
