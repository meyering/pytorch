...
class AOTInductorModelContainer {
 public:
  ...
  void update_constant_buffer(
    ...
    // transferred. If a call in the loop throws (node alloc in insert_or_assign,
    // string ctor), free the not-yet-transferred handles; already-transferred
    // slots are null so they are skipped -- no double free. On success every
    // consumed slot is null, so the caller's map is discarded without freeing
    // (unchanged semantics). model.so-embedded, so stable C ABI only.
    auto num_constants = models_[0]->num_constants();
    try {
      for (size_t idx = 0; idx < num_constants; idx++) {
        auto constant_name =
        ...
        if (it == constants_map.end()) {
          ...
          // a throw in insert_or_assign (which frees the temporary) cannot leave
          // a dangling handle in constants_map to double free.
          it->second = nullptr;
        }
